# 中英对照表 (Glossary)

学车 / 驾考 / 日常开车相关术语.

## 操作 (Maneuvers)

| 中文 | English | 备注 |
| --- | --- | --- |
| 起步 | Pull out / Start moving | 从停车状态驶出 |
| 路口转弯 | Intersection turn / Turning | T 字, 十字路口 |
| 路边停车 | Pull over | 临时靠边停 |
| 平行停车 | Parallel parking | 路考必考 |
| 倒车入库 | Back into a spot / Reverse parking | |
| 三点掉头 | Three-point turn / K-turn | 路太窄不能 U-turn 时 |
| 调头 | U-turn | |
| 变道 | Change lanes / Lane change | |
| 并道 | Merge | 上高速, 入主路 |
| 倒车 | Reverse / Back up | |
| 加速 | Accelerate / Speed up | |
| 减速 | Decelerate / Slow down | |
| 礼让 | Yield | 给行人/对向车 |

## 档位 (Gears)

| 标志 | 中文 | English |
| --- | --- | --- |
| `P` | 停车档 | Park |
| `R` | 倒车档 | Reverse |
| `N` | 空档 | Neutral |
| `D` | 行驶档 | Drive |
| `L` | 低速档 | Low |

## 车的部件 (Car Parts)

| 中文 | English |
| --- | --- |
| 方向盘 | Steering wheel |
| 转向灯 | Turn signal / Blinker |
| 后视镜 (车内中间) | Rearview mirror |
| 后视镜 (左右两侧) | Side mirror / Wing mirror |
| 盲区 | Blind spot |
| 刹车踏板 | Brake (pedal) |
| 油门踏板 | Gas pedal / Accelerator |
| 手刹 / 驻车制动 | Parking brake / Emergency brake / E-brake |
| 仪表盘 | Dashboard |
| 前挡风玻璃 | Windshield |
| 后窗 | Rear window |
| 雨刷 | Wipers |
| 近光灯 / 远光灯 | Low beams / High beams |
| 危险警告灯 | Hazard lights / Hazards |
| 喇叭 | Horn |
| 安全带 | Seatbelt |
| 副驾驶座 | Passenger seat |

## 标志与道路 (Signs & Road)

| 中文 | English | 备注 |
| --- | --- | --- |
| 停车让行 | Stop sign | 红色八角 |
| 让行 | Yield | 倒三角 |
| 单行道 | One Way | |
| 禁止停车 | No Parking | |
| 禁止左转 | No Left Turn | |
| 禁止调头 | No U-turn | |
| 限速 | Speed Limit | |
| 校车 | School bus | 闪红灯时双向都要停 |
| 路缘 / 马路牙子 | Curb | |
| 人行道 (斑马线) | Crosswalk | |
| 人行步道 | Sidewalk | |
| 红绿灯 | Traffic light / Signal | |
| 双黄线 | Double yellow line | 不可压, 不可越 |
| 单黄实线 | Single yellow line | |
| 白色虚线 | White dashed line | 可变道 |
| 中央分隔带 | Median | |
| 转盘 / 环岛 | Rotary | **麻州本地叫 rotary** (其他州 roundabout) |
| 立交桥 | Overpass / Interchange | |
| 高速公路 | Highway / Interstate | I-90, I-93 等 |
| 主路 / 辅路 | Main road / Side street | |
| 上匝道 / 下匝道 | On-ramp / Off-ramp | |
| 出口 | Exit | |
| 收费站 | Toll booth / Toll plaza | MA 用 EZ-Pass 电子收费 |

## 考试与驾照 (Test & Licensing)

| 中文 | English | 备注 |
| --- | --- | --- |
| 麻州机动车管理局 | Registry of Motor Vehicles (RMV) | 麻州的 DMV 叫 RMV |
| 笔试 / 许可证考试 | Written test / Permit test | |
| 视力检查 | Vision test | |
| 路考 | Road test | |
| 学车许可 | Learner's permit | 通过笔试后拿到 |
| 正式驾照 | Driver's license | 通过路考后拿到 |
| 监考官 | Examiner | 路考时坐副驾的考官 |
| 预约 | Appointment / Schedule | RMV 网上预约 |
| 车辆登记 | Vehicle registration | |
| 保险 | Insurance | 路考时车主必须有保险 |

## 行为 / 教练用语 (Driving Actions & Instructor Phrases)

| 中文 | English |
| --- | --- |
| 踩脚刹 | Press / Step on the brake |
| 松脚刹 | Release the brake / Let off the brake |
| 踩油门 | Press the gas / Hit the gas |
| 松油门 | Let off the gas / Ease off |
| 打转向灯 | Turn on the signal / Signal |
| 检查后视镜 | Check the mirror |
| 检查盲区 | Check the blind spot |
| 回头看 | Look over your shoulder |
| 回正方向盘 | Straighten the wheel |
| 方向盘打到底 | Turn the wheel all the way |
| 慢慢来 | Take it easy / Slow down |
| 注意 | Watch out / Pay attention |
| 加快 | Speed up / Faster |
| 跟车太近 | Tailgating |
| 让行人先过 | Let the pedestrian go first |

## 人称 (People)

| 中文 | English |
| --- | --- |
| 教练 | Driving instructor / Instructor |
| 学员 | Student driver |
| 司机 | Driver |
| 行人 | Pedestrian |
| 骑车人 | Cyclist |
| 警察 | Police officer / Cop |
